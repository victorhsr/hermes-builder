...
class DSLProcessorTest {
    ...
    fun `should generate dsl`() {
        ...
        mockkConstructor(HermesRunner::class)
        ...
        every { anyConstructed<HermesRunner>().genDSL(any(), any()) } returns Unit

        val dslProcessor = DSLProcessor()
        dslProcessor.init(processingEnvironment)

        // when
        dslProcessor.process(setOf(annotation), roundEnv)

        // then
        verify { anyConstructed<HermesRunner>().genDSL(classInfoList, filer) }
    }
    ...
    fun `should log invalid annotated elements`() {
        ...
        // then
        verify { messager.printMessage(eq(Diagnostic.Kind.ERROR), eq("${DSLRoot::class.qualifiedName} is supposed to be used in classes, but it was found in: $annotatedElement")) }
    }
